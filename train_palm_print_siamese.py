...
def palm_print_siamese_model():
    feature_extractor = palm_print_model()
...
def get_train_dataset(train_csv, batch_size):
    df = pd.read_csv(train_csv, usecols=['palm_print', 'label'])
    ...
    all_dataframe = all_dataframe[
            all_dataframe.palm_print_left != all_dataframe.palm_print_right]
    ...
    pos_dataframe = all_dataframe[
        all_dataframe.label_left == all_dataframe.label_right][
            ['palm_print_left', 'palm_print_right']]
...
if __name__ == "__main__":
    ...
    train_dataset = get_train_dataset('train.csv', batch_size=args.batch_size)
    val_dataset = get_val_dataset("train.csv", "val.csv", batch_size=args.batch_size)

    # df = pd.read_csv('train.csv', usecols=['palm_print'])
    # num_palm_prints_in_train = df.drop_duplicates().shape[0]
    # steps_per_epoch = (num_palm_prints_in_train**2)//args.batch_size

    # df = pd.read_csv('val.csv', usecols=['palm_print'])
    # num_palm_prints_in_val = df.drop_duplicates().shape[0]
    # validation_steps = (num_palm_prints_in_val**2)//args.batch_size
