...
class PersonIDSequence(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        # signatures = np.array([self.signature_by_file[file] for file in X.face])
        # faces = np.array(list(map(self.load_face, X.face))).astype(np.float32)
        # palm_prints = np.array(list(map(self.load_palm_print, X.palm_print))).astype(np.float32)
        # audios = np.array(list(map(self.load_audio, X.audio))).astype(np.float32)
        # signatures = np.array(list(map(self.load_signature, X.signature))).astype(np.float32)
